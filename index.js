...
function processSum(numberList, callback) { 
  return callback(numberList.reduce(function(accumalator, elements){ 
    return accumalator + elements
  }, 0))
}
...
//function getFullNames(runners) {
 // let newRunners = [];
 // runners.forEach(function(element){newRunners.push(element.last_name + element.first_name)});
  //return newRunners;
//}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  return runners.filter(item => {if(item.shirt_size === tShirtSize) {return true};
  if(item.shirt_size ==! tShirtSize) {return false}})
}
...
function tallyUpDonations(runners) {
 return runners.reduce(function(accumalator, item){return accumalator + item.donation}, 0);
